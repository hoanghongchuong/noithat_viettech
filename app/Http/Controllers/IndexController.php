...
class IndexController extends Controller {
	...
	public function index()
	{
		...
		$cateProjectHome = NewsCate::where('com','du-an')->where('status',1)->where('hot',1)->first();
		$project = $cateProjectHome->projects();
		$setting =DB::table('setting')->select()->where('id',1)->get()->first();
		...
		$services = DB::table('news')->where('com','dich-vu')->take(2)->orderBy('id','desc')->get();
		...
		return view('templates.index_tpl', compact('com','keyword','description','title','img_share','partners','project','categories_home','categories_design_home','news','about','partners','cateProjectHome','services','slogan'));
	}
	...
	{
		$tintuc = DB::table('news')->where('status',1)->where('com','du-an')->orderby('id','desc')->paginate(18);
		$com='thiet-ke';
		// Cấu hình SEO
		$title = "Dự án";
		$keyword = "Dự án";
	...
	{
		$cate = DB::table('news_categories')->where('status',1)->where('com','du-an')->where('alias',$alias)->first();

		if(!empty($cate)){
			$data = DB::table('news')->select()->where('status',1)->where('cate_id',$cate->id)->orderBy('id','desc')->paginate(18);			
			if(!empty($cate->title)){
				$title = $cate->title;
			}else{
				$title = $cate->name;
			...
			$img_share = asset('upload/news/'.$cate->photo);
			// End cấu hình SEO
			return view('templates.project_list', compact('data','cate','keyword','description','title','img_share'));
		}else{
			return redirect()->route('getErrorNotFount');
		}
	}
	...
	{
		$news_detail = DB::table('news')->where('status',1)->where('com','du-an')->where('alias',$alias)->first();		
		if(!empty($news_detail)){			
			...
			$description = $news_detail->description;
			$img_share = asset('upload/news/'.$news_detail->photo);
			return view('templates.project_detail', compact('news_detail','com','keyword','description','title','img_share','postSame','news_cate'));
		}else{
			return redirect()->route('getErrorNotFount');
		}
	}

	public function getThiCong()
	{
		$com = 'thi-cong';
		$title = 'Thi công nội thất';
		$data = DB::table('news')->where('com','thi-cong')->where('status',1)->orderBy('id','desc')->paginate(12);
		return view('templates.thicong', compact('title','com','data'));
	}
	...
	{
		$data = DB::table('news')->where('com','dich-vu')->where('alias',$alias)->first();
		$com ='dich-vu';
		$title = $data->name;
		return view('templates.service_detail', compact('title','com','data'));
	}
